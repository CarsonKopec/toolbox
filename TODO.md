...
- [x] **Phase 3b — Restart supervision.** A tool's `restart` policy (`no`/`on-failure`/`always`, via `config add-tool --restart`) is enforced by a detached `toolbox __supervise` shim that respawns the process with capped backoff; `stop` kills the whole group/tree.
